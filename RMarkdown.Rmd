...
tm1 <- tm_shape(UKNUTS3Map)+
  #setting the base polygon
  tm_polygons("GVA.in..2017", 
              #uusing the breaks defined previous
              breaks=breaksGVA, 
              #using the palette defined previously
              palette = RdBu8,
              #setting the border color to grey
              border.col = "grey",
              #setting the depth of the border as 0.5
              border.alpha= 0.5)+
  #adding a legend
  tm_legend(#setting the title size to small as it cannot be removed
            title.size = 0.01,
            #showing the legend
            show=TRUE, 
            #setting the position to the left of the plot
            position = c(0.05,0.4))+
  tm_layout(#setting the title
            title = "GVA in 2017",
            #setting the size of the title
            title.size = 0.2,
            #setting the title position
            title.position = c("left", "top"),
            #framing the entire plot
            frame=TRUE)+
...
```{r}
#creating a tmap plot showing unemployment rate
tm5 <- 
  #specifying where the data and map should come from
  tm_shape(UKNUTS3Map)+
  #setting the polygon
  tm_polygons(
              #using the unemployment data
              "U.E.rate...Jul.2018.Jun.2019",
              #using the breaks specified before
              breaks=breaksUE,
              #setting the palette as the reverse RdBu pallete set before as a higher value is positive
              palette = ReversedRdBU,
              #setting the border colour as grey
              border.col = "grey",
              #setting the border alpha as 0.5
              border.alpha= 0.5)+
  #adding a legend
  tm_legend(
            #setting the legend size as almost 0 so that it won't be seen
            title.size = 0.01,
            #making sure that the legend is shown
            show=TRUE, 
            #setting the position according to what would be most appropriate
            position = c(0.05,0.3)
            )+
  #setting the layour
  tm_layout(
            ...
            #setting the title position
            title.position = c("left", "top"),
            #formatting the title so that it only shows one digit after the decimal point
  ...
  #adding a compass
  tm_compass(
             #setting the type as rose 
             type = "rose",
             #setting the position of this rose
             position = c(0.75, 0.8,
                          color.light="grey90"),
             #setting the size
             size = 3)+
...
#this is then repeated with the other 5 variables essentially
